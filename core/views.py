...
from core.models import User,studentdetails,studentmarks
...
def printresult(request):
    try:
        ...
        return render(request,'student/print.html',{'pdfs':pdfs})
    except Exception as ex:
        msg="Student Detail not Found"
...
def resetpass(req):
    try:
        ...
        return render(req,'commons/forgotpass.html',{'msg':msg}) 
...
def addstudentsdata(req):
    try:
        ...
        result.gender=req.POST.get("gender")
...
def modstu(req):
    try:
        ...
        if (role=="Admin"):
            ...
            return render(req,'commons/invalid.html',{'msg':msg})
    except Exception as ex:
...
def addmarkview(req):
    try:
        ...
        marksdata.email=req.GET.get("email")
        marksdata.rollno=req.GET.get("rollno")
        marksdata.branch=req.GET.get("branch")
        marksdata.semester=req.GET.get("semester")
        marksdata.subject=req.GET.get("subject")
        marksdata.subjectcode=req.GET.get("subjectcode")
        marksdata.fullmark=req.GET.get("fullmark")
        marksdata.obtainmark=req.GET.get("obtainmark")
        marksdata.pracfullmark=req.GET.get("pracfullmark")
        marksdata.pracobtmark=req.GET.get("pracobtmark")
        marksdata.totalmark=(req.GET.get("totalmark"))
        marksdata.result=req.GET.get("result")
        marksdata.CGPA=req.GET.get("CGPA")
        marksdata.resultdate=req.GET.get("resultdate")
        marksdata.save()
        msg="Marks Updated"       
        return render(req,'base.html',{'msg':msg})
    except Exception as ex:
...
def editmarks(req):
    try:
        if req.method=="GET":
            ...
            marksdata.subject=req.POST.get("subject")
            marksdata.subjectcode=req.POST.get("subjectcode")
            marksdata.fullmark=req.POST.get("fullmark")
            marksdata.obtainmark=req.POST.get("obtainmark")
            marksdata.pracfullmark=req.POST.get("pracfullmark")
            marksdata.pracobtmark=req.POST.get("pracobtmark")
            marksdata.totalmark=(req.POST.get("totalmark"))
            marksdata.result=req.POST.get("result")
            marksdata.CGPA=req.POST.get("CGPA")
    ...
    try:
        if request.method == 'POST' and request.FILES['myfile']:      
            ...
            for dbframe in dbframe.itertuples():
                p="Student@123"
                encryptedpassword=make_password(p)
                obj = studentdetails.objects.create(email=dbframe.email, rollno=dbframe.rollno,  name=dbframe.name,  
                lastname=dbframe.lastname,  fathername=dbframe.fathername, fatherlastname=dbframe.fatherlastname,  
                mobileno=dbframe.mobileno,  gender=dbframe.gender,  dob=dbframe.dob, doa=dbframe.doa, address=dbframe.address,)
                obj2 = User.objects.create(email=dbframe.email, username=dbframe.email, first_name=dbframe.name, last_name=dbframe.lastname,
                password=encryptedpassword, role="Student")           
                obj.save()
                obj2.save()
...
def exceluploadmarks(request):
    try:
        if request.method == 'POST' and request.FILES['myfile']:      
            ...
            dbframe = empexceldata
            for dbframe in dbframe.itertuples():
                obj = studentmarks.objects.create(email=dbframe.email,  rollno=dbframe.rollno,  branch=dbframe.branch,  
                     ...
                     subject2=dbframe.subject2,  subjectcode2=dbframe.subjectcode2,  marks2=dbframe.marks2,  subject3=dbframe.subject3, 
                      subjectcode3=dbframe.subjectcode3,  marks3=dbframe.marks3,  subject4=dbframe.subject4,  subjectcode4=dbframe.subjectcode4, 
                       marks4=dbframe.marks4,  subject5=dbframe.subject5,  subjectcode5=dbframe.subjectcode5,  marks5=dbframe.marks5,  
                        school=dbframe.school, resultdate=dbframe.resultdate)           
                obj.save()
                messages.success(request,'Data Uploded Successfully')
        return HttpResponseRedirect('/')
    except IntegrityError as ex:
...
"""def viewresult(req):
    email=req.POST.get("email")
    details=studentmarks.objects.filter(email=email)
...
def marksheet(req):
    ...
    marks=studentmarks.objects.filter(email=email, semester=semester)
    details=studentmarks.objects.filter(email=email)
    details2=studentdetails.objects.filter(email=email)
    for i in marks:
        m1=int(i.marks1)
        m2=int(i.marks2)
        m3=int(i.marks3)
